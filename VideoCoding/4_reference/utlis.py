...
class BlockMatching():
    ...
    def step(self, frames_prev, target):
        self.raw(frames_prev,target); coord = []; dfd_min = []; blocks = []
        for x in self.best_match:
            coord_tmp = [];dfd_min_tmp = [];block_tmp = []
            l = len(x)

            for j in range(l):
                if j % 3 == 0:
                    coord_tmp.append(x[j])
                elif j % 3 == 1:
                    dfd_min_tmp.append(x[j])
                else:
                    block_tmp.append(x[j]) 

            coord.append(coord_tmp)
            dfd_min.append(dfd_min_tmp)
            blocks.append(block_tmp)

        coord = np.asarray(coord).T.tolist()
        dfd_min = np.asarray(dfd_min).T.tolist()
        blocks = np.asarray(blocks).T.tolist()
        store=[]
        for ele in dfd_min:
            i = ele.index(min(ele))
            store.append([coord[0][i],blocks[0][i]])
    ...
    def raw(self,frames_prev,target):
        """One-step run for given frame pair."""
        for prev in frames_prev:
            self.prev = prev
            self.target = target
            self.shape  = prev.shape

            self.frame2blocks()
            # update block.calculate_mv_amp
    ...
    def ThreeStepSearch(self):
        ...
        for block in self.blocks:
            step1,_ = self.OneStepSearch(block,block.coord,searchAreas[0]) 
            step2,_ = self.OneStepSearch(block,step1,searchAreas[1]) 
            step3, dfd_norm_min = self.OneStepSearch(block,step2,searchAreas[2])
            #print(step3)
            best_match.append(step3)
            best_match.append(dfd_norm_min)
            best_match.append(block)

        self.best_match.append(best_match)
